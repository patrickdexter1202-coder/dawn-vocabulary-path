...
- GitHub：源码、内置词库和构建产物；
- `localStorage`：当前浏览器的学习记录；记录面板默认当天，可查询最近 90 个自然日；
...
GitHub Pages 是只读静态托管，不把会话写入 GitHub 仓库。90 天日期查询完全在浏览器内筛选；底层旧会话不主动删除，完整备份仍以当前浏览器实际数据为准。
...
- 仓库和构建日志中不存在家庭数据；
- 学习记录默认打开当天，前后日与日期输入只能访问最近 90 天且不能选择未来；
